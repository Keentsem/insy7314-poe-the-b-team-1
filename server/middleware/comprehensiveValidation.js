...
const VALIDATION_PATTERNS = {
  ...
  iban: {
    ...
    message: 'Invalid IBAN format - must start with 2 letters, 2 digits, followed by alphanumeric characters'
  },
  ...
  recipientName: {
    ...
    message: 'Recipient name must be 2-100 characters, letters, spaces, hyphens and apostrophes only'
  },
  ...
  reference: {
    ...
    message: 'Reference must be 0-35 characters, alphanumeric with spaces, hyphens, underscores, and periods only'
  },
...
const sanitizeNoSQLOperators = (value) => {
  ...
  const dangerousOperators = ['$where', '$regex', '$gt', '$gte', '$lt', '$lte', '$ne', '$in', '$nin', '$exists', '$type'];
...
const validateSwiftSubmission = [
  body('transactionIds').isArray({ min: 1, max: 50 }).withMessage('Must provide 1-50 transaction IDs'),
  body('transactionIds.*').matches(VALIDATION_PATTERNS.transactionId.regex).withMessage('Invalid transaction ID format'),
];
...
const validateCustomerId = [
  createValidator('id', 'param', VALIDATION_PATTERNS.mongoObjectId),
];
