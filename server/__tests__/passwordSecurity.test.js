...
describe('Password Security - EXCEEDS STANDARD Implementation', () => {
  ...
  describe('Input Sanitization - EXCEEDS STANDARD Protection', () => {
    ...
    test('should reject passwords with null bytes', async () => {
      ...
      await expect(hashPassword(maliciousPassword))
        .rejects
        .toThrow('Password contains invalid null bytes');
    });

    test('should reject excessively long passwords (DoS protection)', async () => {
      const longPassword = 'a'.repeat(MAX_PASSWORD_LENGTH + 1);

      await expect(hashPassword(longPassword))
        .rejects
        .toThrow(`Password cannot exceed ${MAX_PASSWORD_LENGTH} characters`);
    });
    ...
    test('should reject empty or whitespace-only passwords', async () => {
      ...
      await expect(hashPassword('   '))
        .rejects
        .toThrow('Password cannot be only whitespace or control characters');
    ...
    test('should remove dangerous control characters', async () => {
      ...
      await expect(hashPassword(dangerousPassword))
        .rejects
        .toThrow(); // Should reject due to null byte or clean to empty
    });

    test('should validate non-string inputs', async () => {
      await expect(hashPassword(null))
        .rejects
        .toThrow('Password must be a string');

      await expect(hashPassword(undefined))
        .rejects
        .toThrow('Password must be a string');
  ...
  describe('Performance and Security Trade-offs', () => {
    ...
    test('should demonstrate computational cost scaling', async () => {
      ...
      console.log(`Password comparison with ${SALT_ROUNDS} rounds took ${comparisonTime.toFixed(2)}ms`);
    });
  });
});