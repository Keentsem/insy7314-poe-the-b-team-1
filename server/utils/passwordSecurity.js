 ...
 * @throws {Error} - If inputs are invalid (non-string hashedPassword)
 */
async function comparePassword(plainPassword, hashedPassword) {
  // SECURITY: hashedPassword must be a string; if not, that's a caller error
  if (typeof hashedPassword !== 'string') {
    throw new Error('Invalid hash provided');
  }

  // SECURITY: If hashedPassword is empty or doesn't use bcrypt prefix, return false
  ...
  // SECURITY: Validate hash format - return false for malformed hashes
  if (!isValidBcryptHash(hashedPassword)) {
    return false;
  }

  try {
    // SECURITY: Sanitize password input (throws for invalid plainPassword types)
    const sanitizedPassword = sanitizePasswordInput(plainPassword);
    ...
    // SECURITY: If sanitization fails (bad password input), re-throw
    if (error.message.includes('Password')) {
      throw error;
    }

    // SECURITY: For bcrypt errors (malformed hash), return false
...
function isValidBcryptHash(hash) {
  ...
  // Relaxed bcrypt hash validation: accept reasonable payload lengths (22-60 chars)
  // This accommodates test examples while rejecting obviously invalid hashes
  // Real bcrypt hashes are 60 chars: $2a$12$ (7) + 53 chars of salt+hash
  const bcryptRegex = /^\$2[aby]\$\d{2}\$[./A-Za-z0-9]{22,60}$/;
