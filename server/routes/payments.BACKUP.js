...
router.get('/employee/pending', authenticateEmployee, async (req, res) => {
  try {
    ...
    // Log employee access
    createSecurityEvent(
      'employee_view_pending_payments',
      {
    ...
    createSecurityEvent(
      'employee_pending_payments_error',
      {
...
router.get('/employee/all', authenticateEmployee, async (req, res) => {
  try {
    ...
    if (
      status &&
      ['pending', 'verified', 'rejected', 'submitted_to_swift', 'completed', 'failed'].includes(
        status
      )
    ) {
...
router.post(
  ...
  [
    ...
    body('verifierNotes')
      ...
      .matches(/^[a-zA-Z0-9\s.,\-!?]*$/)
      .withMessage(
        'Verifier notes must be 500 characters or less, alphanumeric with basic punctuation only'
      ),
  ],
  handleValidationErrors,
  async (req, res) => {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      createSecurityEvent(
        'employee_swift_submission_error',
        {
          employeeId: req.user?.userId || 'unknown',
          error: error.message,
        },
