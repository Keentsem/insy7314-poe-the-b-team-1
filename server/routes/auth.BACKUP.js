...
router.post(
  '/register',
  [sanitizeRequestBody, validateEmail, validatePassword, handleSanitizedValidationErrors],
  async (req, res) => {
    try {
...
router.get('/employee/profile', authenticateToken, (req, res) => {
  try {
    ...
    const employee = Array.from(employees.values()).find(emp => emp.employeeId === req.user.userId);
