...
async function testSQLInjectionLogin() {
  ...
  console.log('\n--- Analysis ---');
  console.log(
    `Result: ${allBlocked ? '✓ PASS' : '✗ FAIL'} - SQL injection attempts ${allBlocked ? 'blocked' : 'may succeed'}\n`
  );
}
...
async function testXSSInjection() {
  ...
  const xssPayloads = [
    ...
    },
    {
      ...
      description: 'Image event handler',
    },
    {
      email: 'user@example.com',
      username: "'; alert('XSS'); //",
      description: 'JavaScript event injection',
    },
    {
      email: 'user@example.com',
      username: "<iframe src='javascript:alert(1)'>",
