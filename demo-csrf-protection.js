...
async function testWithoutToken() {
  return new Promise(resolve => {
    ...
    const req = https.request(options, res => {
      ...
      console.log(
        `Result: ${res.statusCode === 403 ? '✓ PASS' : '✗ FAIL'} - CSRF protection blocked request without token\n`
      );
      resolve();
    });
...
async function testWithInvalidToken() {
  return new Promise(resolve => {
    ...
    const req = https.request(options, res => {
      ...
      console.log(`Expected: 403 Forbidden`);
      console.log(
        `Result: ${res.statusCode === 403 ? '✓ PASS' : '✗ FAIL'} - CSRF protection rejected invalid token\n`
      );
      resolve();
    });
    ...
    const req = https.request(options, res => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log(`Status Code: ${res.statusCode}`);
          console.log(
            `CSRF Token received: ${response.csrfToken ? response.csrfToken.substring(0, 20) + '...' : 'None'}`
          );
          console.log(`Cookie header: ${res.headers['set-cookie'] ? 'Present' : 'Missing'}`);
          console.log(
            `Result: ${response.csrfToken && res.headers['set-cookie'] ? '✓ PASS' : '✗ FAIL'} - CSRF token endpoint working\n`
          );
        } catch (e) {
