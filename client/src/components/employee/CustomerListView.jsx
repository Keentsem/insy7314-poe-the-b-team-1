...
const CustomerListView = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch(
        API_ENDPOINTS.CUSTOMERS_EMPLOYEE_ALL,
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(
        API_ENDPOINTS.CUSTOMERS_EMPLOYEE_STATS,
        getSecureFetchOptions('GET')
      );
      const data = await response.json();
  ...
  const customerItems = customers.map(customer => (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>{customer.accountNumber}</div>
      </div>
    </div>
  ));
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: selectedCustomer ? '1fr 1fr' : '1fr',
          gap: '24px',
        }}
      >
        {/* Customer List */}
              ...
              <div>
                ...
                <div style={{ marginBottom: '24px' }}>
                  <DetailRow
                    ...
                    value={customerDetails.customer.accountNumber}
                  />
                  <DetailRow
                    icon={<FiCalendar />}
                    label="Member Since"
                    value={new Date(customerDetails.customer.createdAt).toLocaleDateString()}
                  />
                  <DetailRow
                    icon={<FiDollarSign />}
                    label="Account Balance"
                    value={`$${customerDetails.customer.accountBalance.toFixed(2)}`}
                  />
                </div>
                ...
                >
                  <h4 style={{ color: '#e5e7eb', marginBottom: '12px', fontSize: '1rem' }}>
                    Payment Statistics
                  </h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    ...
                    <MiniStat
                      label="Pending"
                      value={customerDetails.stats.pendingPayments}
                      color="#f59e0b"
                    />
                    <MiniStat
                      label="Verified"
                      value={customerDetails.stats.verifiedPayments}
                      color="#10b981"
                    />
                    <MiniStat
                      label="Rejected"
                      value={customerDetails.stats.rejectedPayments}
                      color="#ef4444"
                    />
                  </div>
                  <div
