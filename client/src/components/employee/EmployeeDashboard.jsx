...
const EmployeeDashboardEnhanced = ({ employee, onLogout }) => {
  ...
  const openVerifyDialog = (tx, isApproval) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: handleVerifyTransaction,
      onClose: () => {
        // Close confirm dialog and clear selected transaction
        setConfirmDialog({ ...confirmDialog, isOpen: false });
        setSelectedTransaction(null);
        setVerificationForm({ verified: true, verifierNotes: '' });
      },
  ...
  const openSwiftDialog = () => {
    const selectedTxs = Array.from(selectedForSwift).map(id =>
      transactions.find(tx => tx.transactionId === id)
    ).filter(Boolean);
  ...
  const navItems = [
    {
      ...
      textColor: "#fff",
      links: [
        { label: "Pending Payments", ariaLabel: "View Pending Payments", onClick: () => { setCurrentView('payments'); setActiveTab('pending'); } },
        { label: "All Payments", ariaLabel: "View All Payments", onClick: () => { setCurrentView('payments'); setActiveTab('all'); } },
        { label: "Accepted Archive", ariaLabel: "View Accepted Payments Archive", onClick: () => setCurrentView('accepted') }
      ]
    },
    {
      label: "Customers",
      ...
      textColor: "#fff",
      links: [
        { label: "Customer Directory", ariaLabel: "View Customer Directory", onClick: () => setCurrentView('customers') }
      ]
    },
    {
      label: "Account",
      ...
      textColor: "#fff",
      links: [
        { label: "Profile", ariaLabel: "View Profile", onClick: () => { setCurrentView('payments'); setActiveTab('pending'); } },
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 10, padding: '2rem', paddingTop: '6rem' }}>
          ...
          <>
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              >
                ...
                <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', paddingBottom: '1rem', paddingLeft: '2.5rem', paddingRight: '1.5rem', borderTop: '1px solid rgba(99, 102, 241, 0.2)', display: 'flex', justifyContent: 'flex-start', gap: '32px' }}>
                  <FolderIcon
                    color="#667eea"
                    size={1.5}
                    label="Accepted Payments"
                    count={transactions.filter(t => t.status === 'verified' || t.status === 'completed' || t.status === 'submitted_to_swift').length}
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <Panel>
            <div style={{ padding: '1rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#e5e7eb', fontSize: '0.875rem' }}>
                  <input
                    type="checkbox"
                    checked={autoRefresh}
                    onChange={(e) => {
                      setAutoRefresh(e.target.checked);
                      showToast.info(e.target.checked ? 'Auto-refresh enabled' : 'Auto-refresh disabled');
                    }}
                  />
                  Auto-refresh
                </label>
          ...
          <Panel>
            <div style={{ padding: '1rem' }}>
              <div style={{ display: 'flex', gap: '1rem', marginBottom: showFilters ? '1rem' : 0, flexWrap: 'wrap' }}>
                ...
                {(searchTerm || filterCurrency !== 'all' || filterAmountMin || filterAmountMax) && (
                  <button
              ...
              {/* Advanced Filters */}
              {showFilters && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', paddingTop: '1rem', borderTop: '1px solid #374151' }}>
                  <div>
        ...
        <Panel>
          <div style={{ padding: '1.5rem' }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: '600', color: '#e5e7eb', marginBottom: '1rem' }}>
              {activeTab.charAt(0).toUpperCase() + activeTab.slice(1).replace('_', ' ')} Transactions
            </h2>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '3rem', color: '#9ca3af' }}>
                <p style={{ fontSize: '1.125rem', marginBottom: '0.5rem' }}>No transactions found</p>
                {(searchTerm || filterCurrency !== 'all' || filterAmountMin || filterAmountMax) && (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {currentTransactions.map(tx => (
                        <tr key={tx.transactionId} style={{ borderBottom: '1px solid #1f2937' }}>
                          {activeTab === 'verified' && (
                  ...
                  {selectedTransaction.status === 'pending' && (
                    <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid #374151' }}>
                      ...
                      <div style={{ display: 'flex', gap: '1rem' }}>
                        <button
                          onClick={() => {
                            setVerificationForm({ ...verificationForm, verified: true });
                            openVerifyDialog(selectedTransaction, true);
                          }}
                          disabled={actionLoading}
                          style={{
                            flex: 1,
                        ...
                        <button
                          onClick={() => {
                            setVerificationForm({ ...verificationForm, verified: false });
                            openVerifyDialog(selectedTransaction, false);
                          }}
                          disabled={actionLoading}
                          style={{
                            flex: 1,
