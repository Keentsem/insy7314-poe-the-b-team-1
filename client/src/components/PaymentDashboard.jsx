...
const PaymentDashboard = ({ user, onLogout }) => {
  ...
  const fetchInvoiceCount = async () => {
    try {
      const response = await fetch(API_ENDPOINTS.PAYMENTS, getSecureFetchOptions('GET'));
      ...
      <div className="dashboard-container" style={{ position: 'relative', zIndex: 1 }}>
        <header className="dashboard-header">
          <div className="ascii-header-container" style={{ pointerEvents: 'auto' }}>
            <ASCIIText text="welcome" enableWaves={false} asciiFontSize={0.75} />
          </div>
        ...
        <div className="dashboard-content">
          ...
          {currentView === 'invoices' ? (
            ...
            <div className="dashboard-grid">
              <PaymentForm userId={user.id} onPaymentComplete={handlePaymentComplete} />
              ...
              <div>
                ...
                >
                  <FolderIcon
                    ...
                    count={invoiceCount}
                    onClick={() =>
                      setCurrentView(currentView === 'invoices' ? 'dashboard' : 'invoices')
                    }
                  />
                </div>
              </div>
